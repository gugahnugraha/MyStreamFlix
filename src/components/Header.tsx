...
export default function Header({
  ...
  // Global Outside Click / Touch Tap Listener for Profile Dropdown & Language Menu
  useEffect(() => {
    if (!showProfileMenu && !showLangMenu) return;
      ...
      <div className="flex items-center gap-3 sm:gap-4" id="header-actions">
        ...
        <div className="relative hidden sm:block" ref={langMenuRef}>
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/[0.06] hover:bg-white/[0.12] border border-white/10 hover:border-white/20 text-zinc-200 hover:text-white transition-all cursor-pointer text-xs font-semibold shadow-xs"
          ...
          >
            ...
            <span className="font-medium text-xs">
              {currentLanguage === "id" ? "Bahasa" : currentLanguage === "es" ? "Espanol" : "English"}
            </span>
            <ChevronDown className={`w-3.5 h-3.5 text-zinc-400 transition-transform duration-200 ${showLangMenu ? "rotate-180 text-white" : ""}`} />
          </button>

          {showLangMenu && (
            <div className="absolute right-0 top-full mt-2 w-36 bg-[#111112]/98 backdrop-blur-xl border border-white/[0.1] rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] p-1.5 z-50 animate-in fade-in zoom-in-95 duration-150">
              <button
                onClick={() => {
                  onLanguageChange("id");
                  setShowLangMenu(false);
                ...
                className={`w-full text-left px-3 py-2 text-xs font-semibold rounded-lg flex items-center justify-between transition-colors cursor-pointer ${
                  currentLanguage === "id" ? "text-white bg-white/10" : "text-zinc-300 hover:text-white hover:bg-white/5"
                }`}
                style={currentLanguage === "id" ? { color: brandColor, backgroundColor: `${brandColor}15` } : {}}
              >
                <span>Bahasa</span>
                {currentLanguage === "id" && <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: brandColor }} />}
              </button>
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs font-semibold rounded-lg flex items-center justify-between transition-colors cursor-pointer ${
                  currentLanguage === "en" ? "text-white bg-white/10" : "text-zinc-300 hover:text-white hover:bg-white/5"
                }`}
                style={currentLanguage === "en" ? { color: brandColor, backgroundColor: `${brandColor}15` } : {}}
              >
                <span>English</span>
                {currentLanguage === "en" && <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: brandColor }} />}
              </button>
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 text-xs font-semibold rounded-lg flex items-center justify-between transition-colors cursor-pointer ${
                  currentLanguage === "es" ? "text-white bg-white/10" : "text-zinc-300 hover:text-white hover:bg-white/5"
                }`}
                style={currentLanguage === "es" ? { color: brandColor, backgroundColor: `${brandColor}15` } : {}}
              >
                <span>Espanol</span>
                {currentLanguage === "es" && <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: brandColor }} />}
              </button>
            </div>
          )}
        </div>
